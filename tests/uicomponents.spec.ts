...
test.describe('Form layouts page @block', () => {
    ...
    test('input field', async ({ page }, testInfo) => {
        if (testInfo.retry) {
            //do something during retry
        }

        const usingTheGridEmailInput = page.locator('nb-card', { hasText: "Using the Grid" }).getByRole('textbox', { name: "Email" })

        await usingTheGridEmailInput.fill('test@test.com')
        await expect(usingTheGridEmailInput).toHaveValue('test@test.com') //simple assertion
        await usingTheGridEmailInput.clear()
        await usingTheGridEmailInput.pressSequentially('test2@test.com') // ('test2@test.com', { delay: 100 }) if you want delay input
...
test('list', async ({ page }) => {
    ...
    page.getByRole('listitem') //then the lsit has has li tag
...
test('web tables', async ({ page }) => {
    ...
    //1 get the row by any test in a row
